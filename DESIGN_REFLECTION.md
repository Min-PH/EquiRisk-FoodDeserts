...
This approach operationalizes what I have termed **"Encoding Justice"** — a design principle proposed in my textbook *Artificial Intelligence in Public Health* (Springer Nature, 2026). Encoding Justice holds that justice and equity must be embedded in the architecture of AI-enabled systems before any data enters the pipeline — not treated as post-hoc corrections or retrospective audits. In the EquiRisk-FoodDeserts CMDDS, this principle is visible in every major design decision: BC-7 treats trust as a structural barrier rather than an individual failing; the mandatory NULL output forces the system to acknowledge when it cannot help rather than generating hollow recommendations; the Equity Audit Layer runs at defined intervals with alert thresholds, making inequity detection a system function rather than an optional analysis; and the prioritization rules are evaluated not by throughput but by whether the most vulnerable patients are actually reached. These are not features added after the system was designed — they are the architecture itself. That is what Encoding Justice means in practice.

For a solo researcher or small team working at this scale of complexity, this may be the most practical — and perhaps the only feasible — way to maintain the internal consistency that real-world application demands. For complex public health challenges where clinical, contextual, structural, economic, geographic, temporal, and trust-related variables all interact simultaneously — AI learning within human-defined rules is essential for protecting equity in ways that data-driven approaches alone cannot reliably achieve.
...
Or consider BC-7 (Trust and Engagement Boundaries). An efficiency-optimized system, without explicit equity constraints, would deprioritize hard-to-reach patients — skipping them serves more people per dollar. But those patients often have the greatest historical exposure to system failures. They are exactly the people the system most needs to reach. Projects S4 and A4 force the simulation to grapple with this tension, measuring whether trust-adjusted routing improves equity and for whom.
